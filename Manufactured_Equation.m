...
%solveing for Top and Bottom Boundaries 
W(1,:)=U(1,:);
W(end,:)=U(end,:);
%%
DX = 2*pi/(M+1);
...
error_iterations
figure
subplot(1,2,1),surf(U), xlabel('Y axis'), ylabel('X axis'), zlabel('Z axis'), title('F=1+X^2+Y^2')
subplot(1,2,2),contour(U), xlabel('Y axis'), ylabel('X axis'), title('F=1+X^2+Y^2')
%figure
